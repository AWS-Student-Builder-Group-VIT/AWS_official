...
test('exposes every current React game through a unique internal route', () => {
  assert.deepEqual(
    games.map(({ slug }) => slug),
    ['flappy-bird', 'fruit-ninja', 'snake', 'wordle', 'crack-the-code', 'detective-crime', 'level-devil', 'morse', 'pacman', 'mario-kart', 'watergirl-fireboy', 'asteroid-command', 'gunshot-roulette', 'hack-type'],
  );
