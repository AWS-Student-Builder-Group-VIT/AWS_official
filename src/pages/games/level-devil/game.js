/* ============================================================
   ...
   ============================================================ */
(function createAWSLevelDevilEngineScope() {
"use strict";

/* CORE_START */
const AWSLevelDevilCore = (() => {
  const VERSION = 1;
  ...
  const makeId = () => globalThis.crypto?.randomUUID?.() || `level-devil-${Date.now()}-${Math.random().toString(16).slice(2)}`;

  function pointsForAttempt(attempt) {
    if (!Number.isInteger(attempt) || attempt < 1 || attempt > MAX_ATTEMPTS) return 0;
    return attempt <= 3 ? 10 : 13 - attempt;
  }

  function createState({ official = true, now = Date.now(), attemptId = makeId(), level = 0 } = {}) {
    const currentLevel = official ? 0 : Math.max(0, Math.min(TOTAL_LEVELS - 1, Number(level) || 0));
    return {
      version: VERSION,
      attemptId,
  ...
  function enterLevel(state) {
    ...
    return { ...clone(state), inLevel: true };
  }

  function consumeAttempt(state, reason = "death", now = Date.now()) {
    if (!state || state.status !== "active") return state;
    const next = clone(state);
    next.totalDeaths += 1;
    next.lastAttemptReason = reason;
    next.inLevel = false;
    if (!next.official) return next;

    if (next.currentAttempt >= MAX_ATTEMPTS) {
      next.status = "failed";
      next.failedLevel = next.currentLevel + 1;
      next.completedAt = now;
      next.levelResults.push({ level: next.currentLevel + 1, attempts: MAX_ATTEMPTS, points: 0, outcome: "failed" });
      return next;
    }
  ...
  function completeLevel(state, now = Date.now()) {
    if (!state || state.status !== "active") return state;
    const next = clone(state);
    const points = next.official ? pointsForAttempt(next.currentAttempt) : 0;
    next.levelResults.push({
      level: next.currentLevel + 1,
      attempts: next.currentAttempt,
      points,
      outcome: "complete",
    });
    next.score += points;
    ...
    next.inLevel = false;
    if (!next.official) return next;
    if (next.currentLevel === TOTAL_LEVELS - 1) {
      next.status = "complete";
      next.completedAt = now;
    } else {
      next.currentLevel += 1;
      next.currentAttempt = 1;
      next.lastAttemptReason = null;
    }
    return next;
  }
  ...
  function isValidState(state) {
    return Boolean(
      state && state.version === VERSION && typeof state.attemptId === "string" &&
      state.official === true && ["active", "complete", "failed"].includes(state.status) &&
      Number.isInteger(state.currentLevel) && state.currentLevel >= 0 && state.currentLevel < TOTAL_LEVELS &&
      Number.isInteger(state.currentAttempt) && state.currentAttempt >= 1 && state.currentAttempt <= MAX_ATTEMPTS &&
      Number.isFinite(state.score) && state.score >= 0 && state.score <= MAX_SCORE &&
      Number.isInteger(state.completedLevels) && state.completedLevels >= 0 && state.completedLevels <= TOTAL_LEVELS &&
      Array.isArray(state.levelResults) && Number.isInteger(state.totalDeaths) && state.totalDeaths >= 0 &&
      Number.isFinite(state.startedAt) && typeof state.inLevel === "boolean"
    );
  }

  function serializeState(state) { return JSON.stringify(state); }

  function restoreState(raw, now = Date.now()) {
    try {
      const state = JSON.parse(raw);
      if (!isValidState(state)) return null;
      return state.status === "active" && state.inLevel
        ? consumeAttempt(state, "reload", now)
        : state;
    } catch {
      return null;
    }
  ...
  const shouldPersist = (state) => Boolean(state?.official);
  const canStartOfficial = (state) => !state?.official;
  const shouldNotifyCompletion = (state) => Boolean(
    state?.official && state.status !== "active" && !state.completionNotifiedAt
  );

  function buildCompletionPayload(state) {
    return {
      game: "aws-level-devil",
      version: VERSION,
      ...
      failedLevel: state.failedLevel,
      levelResults: clone(state.levelResults),
  ...
  return {
    VERSION, TOTAL_LEVELS, MAX_ATTEMPTS, MAX_SCORE,
    pointsForAttempt, createState, enterLevel, consumeAttempt, completeLevel,
    serializeState, restoreState, shouldPersist, canStartOfficial,
    shouldNotifyCompletion, buildCompletionPayload,
  };
})();
globalThis.AWSLevelDevilCore = AWSLevelDevilCore;
/* CORE_END */
...
playerLogo.src = globalThis.__AWSLevelDevilPlayerLogoUrl || "aws-chip-player.png";
...
// ---------------------------------------------------------------- theme
const theme = {
    paper: "#080d15", paper2: "#111824", ink: "#f4f6f8",
    grid: "rgba(255,153,0,0.055)", vignette: "rgba(0,0,0,0.38)",
    danger: "#ff9900", accent: "#ffb84d", door: "#ff9900",
    blood: "#ff9900", bloodDark: "#c46f00", dust: "#5d6674",
    metal: "#263143", crack: "rgba(8,13,21,0.55)", wipe: "#05080d",
...
class Door {
  ...
  draw() {
    ...
    ctx.fillStyle = theme.ink;
    ctx.fillRect(4, 0, 4, this.h);
    const cell = 10;
    for (let row = 0; row < 4; row++) {
      for (let col = 0; col < 3; col++) {
        ctx.fillStyle = (row + col) % 2 === 0 ? theme.door : "#111824";
        ctx.fillRect(8 + col * cell, 3 + row * cell, cell, cell);
      }
    }
    ctx.fillStyle = theme.door;
    ctx.beginPath();
    ctx.arc(6, this.h, 7, Math.PI, 0);
    ctx.fill();
    ctx.restore();
  }
}
...
const Game = {
  ...
  restartLevel(manual = false) {
    if (manual && !consumeCurrentAttempt("restart")) return;
  ...
  update(dt) {
    ...
    if (this.state === "play") this.updatePlay(dt);
      ...
      if (this.deathT > 0.85) {
        this.startWipe(() => {
          if (sessionMode === "official" && officialState?.status !== "active") showResults(officialState);
          else { this.loadLevel(this.levelIndex); markLevelEntered(); this.state = "play"; }
        });
      ...
      if (this.winT > 0.8) {
        ...
        if (sessionMode === "official" && officialState?.status === "complete") {
          this.startWipe(() => showResults(officialState));
        } else if (sessionMode === "practice") {
          this.startWipe(() => showPracticeResult(this.levelIndex));
        } else {
          this.startWipe(() => {
            this.loadLevel(officialState.currentLevel);
            markLevelEntered();
            this.state = "play";
          });
        }
  ...
  draw() {
    ...
    if (this.level) {
      ...
      if (this.state === "win" || this.state === "betweenLevels") {
        ...
        ctx.textAlign = "center";
        const finalOfficial = sessionMode === "official" && officialState?.status === "complete";
        ctx.fillText(finalOfficial ? "RUN COMPLETE" : sessionMode === "practice" ? "FLAG CAPTURED" : "FINE. NEXT.", W / 2, H / 2 - 40);
        ctx.globalAlpha = 1;
      }
    }
    ctx.restore();
  ...
  drawPlayer() {
    ...
    roundRect(-p.w / 2, -p.h, p.w, p.h, 6);
    ctx.save();
    ctx.clip();
    ctx.fillStyle = "#111824";
    ctx.fillRect(-p.w / 2, -p.h, p.w, p.h);
    if (playerLogo.complete && playerLogo.naturalWidth) {
      ctx.drawImage(playerLogo, -p.w / 2, -p.h, p.w, p.h);
    } else {
      ctx.fillStyle = theme.accent;
      ctx.fillRect(-8, -p.h + 7, 16, p.h - 14);
      ctx.fillRect(-p.w / 2 + 5, -p.h / 2 - 3, p.w - 10, 6);
    }
    ctx.restore();
    ctx.strokeStyle = "rgba(255,153,0,.9)";
    ctx.lineWidth = 1.5;
    ctx.stroke();
...
// ---------------------------------------------------------------- official attempt / DOM
const STORAGE_KEY = "aws-level-devil:v1";
const VISIBLE_LEVELS = AWSLevelDevilCore.TOTAL_LEVELS;
let officialState = null;
let sessionMode = "landing";
...
const practicePanelEl = document.getElementById("practice-panel");
...
function updateHud() {
  const official = sessionMode === "official";
  document.getElementById("hud-mode").textContent = official ? "OFFICIAL" : "PRACTICE";
  document.getElementById("hud-score").textContent = official ? officialState?.score ?? 0 : "—";
  document.getElementById("hud-attempt").textContent = official ? `${officialState?.currentAttempt ?? 1}/7` : "∞";
  document.getElementById("hud-deaths").textContent = official ? officialState?.totalDeaths ?? 0 : practiceDeaths;
}

function notifyOfficialCompletion() {
  if (!AWSLevelDevilCore.shouldNotifyCompletion(officialState)) return;
  const payload = AWSLevelDevilCore.buildCompletionPayload(officialState);
  globalThis.dispatchEvent(new CustomEvent("aws-level-devil:complete", { detail: payload }));
  if (typeof globalThis.AWSLevelDevilOnComplete === "function") {
    try { globalThis.AWSLevelDevilOnComplete(payload); }
    catch (error) { console.error("AWSLevelDevilOnComplete failed:", error); }
  }
  try { globalThis.__AWSLevelDevilReactOnComplete?.(payload); }
  catch (error) { console.error("React onComplete failed:", error); }
  officialState = { ...officialState, completionNotifiedAt: Date.now() };
  persistOfficial();
}

function markLevelEntered() {
  if (sessionMode === "official") {
    officialState = AWSLevelDevilCore.enterLevel(officialState);
    persistOfficial();
  }
  updateHud();
}

function consumeCurrentAttempt(reason) {
  if (sessionMode === "practice") {
    practiceDeaths += 1;
    Game.deaths = practiceDeaths;
    updateHud();
    return true;
  }
  if (sessionMode !== "official" || !officialState) return true;
  officialState = AWSLevelDevilCore.consumeAttempt(officialState, reason, Date.now());
  Game.deaths = officialState.totalDeaths;
  persistOfficial();
  updateHud();
  if (officialState.status !== "active") {
    notifyOfficialCompletion();
    if (reason === "restart") {
      Game.state = "transition";
      Game.startWipe(() => showResults(officialState));
    }
    return false;
  }
  return true;
...
function completeCurrentLevel() {
  if (sessionMode !== "official") return;
  officialState = AWSLevelDevilCore.completeLevel(officialState, Date.now());
  Game.deaths = officialState.totalDeaths;
  persistOfficial();
  updateHud();
  notifyOfficialCompletion();
...
function buildLevelGrid() {
  ...
  for (let i = 0; i < VISIBLE_LEVELS; i++) {
    ...
    b.textContent = i + 1;
    b.setAttribute("aria-label", `Practice level ${i + 1}: ${LEVELS[i].name}`);
    b.addEventListener("click", () => startGame(i, "practice"));
    grid.appendChild(b);
  }
}

function startGame(i, mode) {
  if (!Number.isInteger(i) || i < 0 || i >= VISIBLE_LEVELS) return;
  AudioFX.init();
  if (IS_TOUCH && !document.fullscreenElement) toggleFullscreen();
  sessionMode = mode;
  document.body.classList.add("gameplay");
  if (mode === "practice") practiceDeaths = 0;
  menuEl.classList.add("hidden");
  endEl.classList.add("hidden");
  hudEl.classList.remove("hidden");
  setTouchControlsVisible(true);
  Game.deaths = mode === "official" ? officialState.totalDeaths : practiceDeaths;
  Game.loadLevel(i);
  Game.state = "play";
  Game.wipe = 1;
  Game.wipeDir = -1;
...
function startOrResumeOfficial() {
  if (!officialState) officialState = AWSLevelDevilCore.createState({ official: true });
  if (officialState.status !== "active") { showResults(officialState); return; }
  persistOfficial();
  startGame(officialState.currentLevel, "official");
}

function showLanding(openPractice = false) {
  sessionMode = "landing";
  ...
  if (!officialState) {
    button.textContent = "START OFFICIAL RUN";
    note.textContent = "One official run is stored in this browser.";
  } else if (officialState.status === "active") {
    button.textContent = `RESUME LEVEL ${officialState.currentLevel + 1}`;
    note.textContent = `Official score ${officialState.score}/50 · attempt ${officialState.currentAttempt}/7.`;
  } else {
    button.textContent = "VIEW OFFICIAL RESULT";
    note.textContent = `Official result locked at ${officialState.score}/50.`;
  }
  practicePanelEl.classList.toggle("hidden", !(openPractice && officialState?.status !== "active"));
}
...
function showResults(state) {
  ...
  Game.state = "end";
  document.getElementById("result-kicker").textContent = "OFFICIAL RUN // LOCKED";
  document.getElementById("result-title").textContent = state.status === "complete" ? "RUN COMPLETE" : "RUN TERMINATED";
  document.getElementById("result-message").textContent = state.status === "complete"
    ? "All five flags captured. The devil has been contained."
    : `Level ${state.failedLevel} consumed all seven attempts.`;
  document.getElementById("end-score").textContent = state.score;
  document.querySelector(".result-score span").textContent = "/ 50 POINTS";
  document.getElementById("end-levels").textContent = `${state.completedLevels}/5`;
  document.getElementById("end-deaths").textContent = state.totalDeaths;
  document.getElementById("end-status").textContent = state.status.toUpperCase();
  const byLevel = new Map(state.levelResults.map((result) => [result.level, result]));
  document.getElementById("result-breakdown").innerHTML = Array.from({ length: VISIBLE_LEVELS }, (_, i) => {
    const result = byLevel.get(i + 1);
    return `<article><small>LEVEL ${i + 1}</small><b>${result ? `${result.points} PTS · ${result.attempts} TRY` : "—"}</b></article>`;
  }).join("");
}

function showPracticeResult(levelIndex) {
  document.body.classList.remove("gameplay");
  hudEl.classList.add("hidden");
  setTouchControlsVisible(false);
  endEl.classList.remove("hidden");
  Game.state = "end";
  document.getElementById("result-kicker").textContent = "PRACTICE MODE // COMPLETE";
  document.getElementById("result-title").textContent = "FLAG CAPTURED";
  document.getElementById("result-message").textContent = `Level ${levelIndex + 1} cleared. Practice never changes the official result.`;
  document.getElementById("end-score").textContent = "—";
  document.querySelector(".result-score span").textContent = "PRACTICE POINTS";
  document.getElementById("end-levels").textContent = `${levelIndex + 1}/5`;
  document.getElementById("end-deaths").textContent = practiceDeaths;
  document.getElementById("end-status").textContent = "PRACTICE";
  document.getElementById("result-breakdown").innerHTML = `<article><small>LEVEL ${levelIndex + 1}</small><b>CLEARED</b></article>`;
}

document.getElementById("play-btn").addEventListener("click", startOrResumeOfficial);
document.getElementById("practice-btn").addEventListener("click", () => showLanding(true));
document.getElementById("practice-close").addEventListener("click", () => practicePanelEl.classList.add("hidden"));
document.getElementById("back-btn").addEventListener("click", () => {
  if (typeof globalThis.__AWSLevelDevilReactOnExit === "function") globalThis.__AWSLevelDevilReactOnExit();
  else showLanding(false);
});
document.getElementById("brand-home").addEventListener("click", (event) => {
  event.preventDefault();
  if (Game.state === "play" && sessionMode === "official") consumeCurrentAttempt("leave");
  showLanding(false);
});
...
function fit() {
  ...
  const portraitPanel = vw < 600 && vw < vh;
  for (const el of [menuEl, endEl]) {
    const panelWidth = portraitPanel ? vw - 24 : cw;
    const panelHeight = portraitPanel ? vh - 96 : ch;
    el.style.width = panelWidth + "px";
    el.style.height = panelHeight + "px";
    el.style.left = portraitPanel ? "12px" : `calc(50% - ${cw / 2}px)`;
    el.style.top = portraitPanel ? "72px" : `calc(50% - ${ch / 2}px)`;
  }
...
wireTopbar();
buildLevelGrid();
officialState = loadOfficial();
if (officialState) persistOfficial();
notifyOfficialCompletion();
showLanding(false);
fit();
...
frameHandle = requestAnimationFrame(frame);
globalThis.AWSLevelDevilDestroy = () => {
