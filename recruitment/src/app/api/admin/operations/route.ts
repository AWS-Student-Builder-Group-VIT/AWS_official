...
export async function GET(request: Request) {
  ...
  const { data: adminUser } = await admin.from('admin_users').select('id,email,name,role').eq('id', authData.user.id).maybeSingle();
  if (!adminUser) return NextResponse.json({ error: 'Administrator access required.' }, { status: 403 });
